...
def get_parsed_arguments():
    ...
    parser.add_argument('--yell', '-y', action='store_true' ,help='makes the noise in all caps')
    parsed_arguments = parser.parse_args()
    return parsed_arguments

def get_animal_noise(language, should_yell, animal):
    ...
    if should_yell:
        animal_noise = animal_noise.upper()
...
def main():
    ...
    for animal in arguments.animals:
        noise = get_animal_noise(arguments.language, arguments.yell, animal)
        if noise:
